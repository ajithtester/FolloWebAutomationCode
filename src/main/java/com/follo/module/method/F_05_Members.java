...
public class F_05_Members extends Baseclass {
	...
	public static void NegativeInviteMembers() throws Throwable {
		...
		ReportConfig.Givenlogpass("User invites already invited member","Invite members button is clicked");
		...
		for(int invite =1; invite<=1; invite++)		{	
			...
			Wait(7000);
			TypeDataInTheField("EnterMailId", MembersLocators, ReadFrom.Excel(invite, 0, ReadFrom.InviteMembers));
			ReportConfig.Givenlogpass("User invites already invited member", "Email id entered : " + ReadFrom.Excel(invite, 0, ReadFrom.InviteMembers));
			...
			ReportConfig.Givenlogpass("User invites already invited member" ,"Close button is clicked");
		}
