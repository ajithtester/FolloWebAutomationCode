...
public class F_08_DeliveryCalendar extends Baseclass {
	...
	public static void Adddelivery() throws Throwable {
		PrintError("Add delivery calendar Started");
...
//
//
//			Delivery.until(new Function<WebDriver, WebElement>() {
//				public WebElement apply(WebDriver driver) {
...
//			}
	...
	public static void SearchDelivery() throws Throwable {
		...
		PrintError("Search delivery calendar started");
...
//						String IssueSummary = "Searched value is : " + ReadFrom.Excel(SearchDelivery, 2, ReadFrom.SearchDelivery) + "but displayed value is : " + DeliveryRequestName ;
...
//						pt_Integration.CreateStory.post_Story(Storydescription , IssueSummary );
	...
	public static void FilterDelivery() throws Throwable {
		...
		PrintError("Filter delivery calendar started");
	...
	public static void EditDelivery() throws Throwable {
		...
		PrintError("Edit delivery calendar started");
		...
		for(int EditDelivery =1; EditDelivery<=ReadFrom.rowcount; EditDelivery++)	{
			...
			Wait(2000);
			Click("SearchIcon", DeliveryCalendarLocators);
			Wait(2000);
			TypeDataInTheField("Search", DeliveryCalendarLocators, ReadFrom.Excel(EditDelivery, 2, ReadFrom.EditDelivery));
			Wait(7000);
...
//					}	
	...
	public static void PushToVoid() throws Throwable {
		...
		PrintError("Push delivery calendar started");
		...
		for(int Void =1; Void<=ReadFrom.rowcount; Void++)	{
			...
			Click("SearchIcon", DeliveryCalendarLocators);
			Wait(2000);
			TypeDataInTheField("Search", DeliveryCalendarLocators, ReadFrom.Excel(Void, 2, ReadFrom.Void));
			Wait(7000);
...
//					}	
	...
	public static void AddAttcahment() throws Throwable {
		...
		PrintError("Add attcahment delivery calendar started");
		...
		for(int AddAttcahment =1; AddAttcahment<=ReadFrom.rowcount; AddAttcahment++)	{
			...
			SearchMonth(AddAttcahment, ReadFrom.AttachmentDeliveryCalendar);
			SearchYear(AddAttcahment, ReadFrom.AttachmentDeliveryCalendar);
			...
			Click("SearchIcon", DeliveryCalendarLocators);
			Wait(2000);
			TypeDataInTheField("Search", DeliveryCalendarLocators, ReadFrom.Excel(AddAttcahment, 2, ReadFrom.AttachmentDeliveryCalendar));
			Wait(7000);
			...
			for(int DeliveryLists =1; DeliveryLists<=1; DeliveryLists++)		{
				...
				String ViewInfo =	ReadFrom.Excel(AddAttcahment, 2, ReadFrom.AttachmentDeliveryCalendar);
...
Wait(6000);
	...
	public static void ViewDeliveryInfo() throws Throwable {
		PrintError("View delivery calendar started");
		...
		for(int ViewDelivery =1; ViewDelivery<=ReadFrom.rowcount; ViewDelivery++)	{
			...
			Click("SearchIcon", DeliveryCalendarLocators);
			Wait(2000);
			TypeDataInTheField("Search", DeliveryCalendarLocators, ReadFrom.Excel(ViewDelivery, 2, ReadFrom.ViewDelivery));
			Wait(7000);
	...
	public static void AddComment() throws Throwable {
		...
		PrintError("Add comment delivery calendar started");
		...
		for(int Comments =1; Comments<=ReadFrom.rowcount; Comments++)	{
			...
			Click("SearchIcon", DeliveryCalendarLocators);
			Wait(2000);
			TypeDataInTheField("Search", DeliveryCalendarLocators, ReadFrom.Excel(Comments, 2, ReadFrom.Comments));
			Wait(7000);
	...
	public static void ViewHistory() throws Throwable {
		PrintError("View history delivery calendar started");
		ReadFrom.DynamicExcel(ReadFrom.ViewDelivery);
	...
	public static void Deliver() throws Throwable {
		PrintError("Deliver delivery calendar started");
		...
		for(int DeliverRequest =1; DeliverRequest<=ReadFrom.rowcount; DeliverRequest++)	{
			...
			Click("SearchIcon", DeliveryCalendarLocators);
			Wait(2000);
			TypeDataInTheField("Search", DeliveryCalendarLocators, ReadFrom.Excel(DeliverRequest, 2, ReadFrom.DeliverRequest));
			Wait(7000);
	...
	public static void ApproveOrReject() throws Throwable {
		PrintError("Approve Or Reject delivery calendar started");
		...
		for(int ApproveOrReject =1; ApproveOrReject<=ReadFrom.rowcount; ApproveOrReject++)	{
			...
			Click("SearchIcon", DeliveryCalendarLocators);
			Wait(2000);
			TypeDataInTheField("Search", DeliveryCalendarLocators, ReadFrom.Excel(ApproveOrReject, 2, ReadFrom.ApproveOrReject));
			Wait(7000);
			...
			//			Wait<WebDriver> Edit = new FluentWait<WebDriver>(driver)
			//					.withTimeout(Duration.ofSeconds(120))
			//					.pollingEvery(Duration.ofSeconds(10))
			//					.ignoring(NoSuchElementException.class);
			//
			//
			//			Edit.until(new Function<WebDriver, WebElement>() {
			//				public WebElement apply(WebDriver driver) {
			//					return driver.findElement(By.xpath("//button[normalize-space()='Edit']"));
			//				}
			//			});
...
//			});
			...
			for (WebElement webElement : Text) {
				...
				if(IsEqual(Name, EditName)) {		
					JsClick(webElement);
					ReportConfig.PassedLogInfo("User approve or reject the request",Name + " : is clicked");
					Print(Name + " : is clicked");
				}

			}

			//			Wait<WebDriver> Expand = new FluentWait<WebDriver>(driver)
			//					.withTimeout(Duration.ofSeconds(120))
			//					.pollingEvery(Duration.ofSeconds(10))
			//					.ignoring(NoSuchElementException.class);
			//
			//
			//			Expand.until(new Function<WebDriver, WebElement>() {
			//				public WebElement apply(WebDriver driver) {
			//					return driver.findElement(By.xpath("//div[1]/div[1]/span/span/em"));
			//				}
			//			});
		...
		//	if(IsElementDisplayed("ViewDetails", DeliveryCalendarLocators)) {
				...
				if (IsEqual(Status, "Approved")) {
					...
					Wait(6000);
				//	Click("ApprovedSuccessfully", DeliveryCalendarLocators);
					...
					Wait(6000);
				//	Click("DeclinedSuccessfully", DeliveryCalendarLocators);
	...
	public static void AddDeliverRequestError() throws Throwable {
		PrintError("AddDeliverRequestError delivery delivery calendar Started");

		for(int AddDelivery =1; AddDelivery<=1; AddDelivery++)		{	
			...
			{
				PrintError("Add button is not clicked");
				ReportConfig.Givenlogpass("User uploads invalid date time in Delivery Calendar","Add button is not clicked");
			}
			...
			if(IsElementDisplayed("Description", DeliveryCalendarLocators)) {
				Print("Delivery popup is opened");
				ReportConfig.Givenlogpass("User uploads invalid date time in Delivery Calendar","Delivery popup is opened");
			}
			...
			//				String shortName = "dc";
			//		    	
			//		    	TypeDataInTheField("Description", DeliveryCalendarLocators, shortName);
			//		    	
			...
			try {
				...
				{
					Print("Delivery request equipment is selected");
					ReportConfig.Givenlogpass("User uploads invalid date time in Delivery Calendar","Delivery request equipment is selected");
			...
			String InvalidTime =	GetText("InvalidTime", DeliveryCalendarLocators);
