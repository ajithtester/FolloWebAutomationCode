...
public class F_11_QueuedRequest extends Baseclass {
	...
	public static void DeleteRequest() throws Throwable {
		PrintError("Delete queued request started");
			...
			for(int List =1; List<=rowcount; List++)	{
				...
				if(IsEqual(QueuedRequestName, DeleteIt)) {
					...
					if(IsElementDisplayed("DeleteSuccessfull", QueuedRequestLocators)){
						...
						String IssueSummary = "Queued Delivery Request Deleted Successfully is not displayed" ;
						String Storydescription = "Delete Queued request" ;
	...
	public static void SearchQueuedRequest() throws Throwable {
		...
		PrintError("Search queued request started");
...
//			
//					String IssueSummary = "Searched value is : " +  ReadFrom.Excel(SearchQueuedRequest, 0, ReadFrom.SearchQueuedRequest) + "but displayed value is : " + QueuedRequestname ;
//					String Storydescription = "Search queued request" ;
//					pt_Integration.CreateStory.post_Story(Storydescription , IssueSummary );
	...
	public static void FilterQueuedRequest() throws Throwable {
		PrintError("Filter queued request started");
			...
			for(int DeliveryLists =1; DeliveryLists<=rowcount; DeliveryLists++)		{
				...
				if(IsEqual(DeliveryName, EditIt)) {
					...
					if(IsElementDisplayed("DeliveryUpdated", QueuedRequestLocators)) {
						...
						String IssueSummary = "Delivery Request Updated Successfully popup is not displayed" ;
						String Storydescription = "Search queued request" ;
						pt_Integration.CreateStory.post_Story(Storydescription , IssueSummary );
					}	
	...
	public static void InvalidFileError() throws Throwable {
		PrintError("InvalidFileError queued request started");
		Wait(2000);

		Click("ImportMultiple", QueuedRequestLocators);

		Print("Import Multiple button is clicked");
		ReportConfig.Givenlogpass("User uploads invalid file in QueuedRequest" ,"Import Multiple button is clicked");
		...
		Click("ClosePopup", QueuedRequestLocators);

		Print("Close button is clicked");
		ReportConfig.Givenlogpass("User uploads invalid file in QueuedRequest" ,"Close button is clicked");
