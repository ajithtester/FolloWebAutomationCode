...
public class Baseclass {
	...
	public static String AppProperties =  System.getProperty("user.dir")
			+ "//src//test//resources//PropertiesFile//App.properties" ;
	...
	public static String DashboardLocators = "./src/test/resources/PropertiesFile/Follo_15_Dashboard.properties";
	public static String ConcreteCalendarLocators = "./src/test/resources/PropertiesFile/Follo_16_ConcreteCalendar.properties";
	public static String ConcreteRequestLocators = "./src/test/resources/PropertiesFile/Follo_17_ConcreteRequest.properties";
	public static String NotificationLocators = "./src/test/resources/PropertiesFile/Follo_18_NotificationPreference.properties";
	...
	public static void verifyLinkActive(String linkUrl) throws Throwable
	{
		try 
		{
			...
			{
				ReportConfig.Givenlogpass("Broken Links in the Login page to be checked" ,linkUrl+" - "+httpURLConnect.getResponseMessage());
			}
			if(httpURLConnect.getResponseCode()==HttpURLConnection.HTTP_NOT_FOUND)  
			{
				Print(linkUrl+" - "+httpURLConnect.getResponseMessage() + " - "+ HttpURLConnection.HTTP_NOT_FOUND);
				ReportConfig.Givenlogfail("Broken Links in the Login page to be checked" ,linkUrl + " >> " + httpURLConnect.getResponseCode() + " >> " +httpURLConnect.getResponseMessage());
			}
	...
	public static void verifyImageActive(String ImageSrc) throws Throwable
	{
		try 
		{
			...
			if(httpURLConnect.getResponseCode()==200)
			{
				Print(ImageSrc+" - "+httpURLConnect.getResponseMessage());
			}
			if(httpURLConnect.getResponseCode()==HttpURLConnection.HTTP_NOT_FOUND)  
			{
				Print(ImageSrc+" - "+httpURLConnect.getResponseMessage() + " - "+ HttpURLConnection.HTTP_NOT_FOUND);
				ReportConfig.Givenlogfail("Broken images in the Login page to be checked" ,ImageSrc + " >> " + httpURLConnect.getResponseCode() + " >> " +httpURLConnect.getResponseMessage());
			}
	...
	public static boolean IfContains(String ActualValue, String ExpectedValue ) throws Throwable {
	...
	public static void EnterKey (String Locator,String locatorfile) throws Throwable {
	...
	public boolean isElementPresent(WebDriver driver, By element) throws Throwable {
		...
		try {
			...
			Print("Exception is Element Is not Present");
		}
		return presenceFlag;
	}

	// select Radio Button
	public void selectTheRadioButton(WebElement element)throws Throwable {
		try {
			if (element.isSelected()) {
				Print("Checkbox: " + element + "is already selected");
			} else {
				// Select the checkbox
				element.click();
			}
		} catch (Exception e) {
			Print("Unable to select the checkbox: " + element);
		}

	}

	// Dselect the RadioButton
	public void deSelectTheRadioButton(WebElement element) throws Throwable {
		try {
			if (element.isSelected()) {
				// De-select the checkbox
				element.click();
			} else {
				Print("Checkbox: " + element + "is already deselected");
			}
		} catch (Exception e) {
			Print("Unable to deselect checkbox: " + element);
		}
	}

	// select the CheckBox
	public void selectTheCheckbox(WebElement element) throws Throwable{
		try {
			if (element.isSelected()) {
				Print("Checkbox: " + element + "is already selected");
			} else {
	...
	public void deSelectTheCheckbox(WebElement element) throws Throwable {
		try {
			if (element.isSelected()) {
				// De-select the checkbox
				element.click();
	...
	public void DissmissAlert() throws Throwable {
		Alert element = driver.switchTo().alert();
		String alertText = element.getText();
		Print(alertText);
		element.dismiss();
	...
	// Hover and Click
	public static void MouseOver(WebElement element) throws Throwable{
		try {
	...
	public static void doubleClick(WebElement element) throws Throwable {
		try {
			Actions action = new Actions(driver).doubleClick(element);
			action.build().perform();

			Print("Double clicked the element");
		} catch (StaleElementReferenceException e) {
			Print("Element is not attached to the page document " + e.getStackTrace());
		} catch (NoSuchElementException e) {
			Print("Element " + element + " was not found in DOM " + e.getStackTrace());
		} catch (Exception e) {
			Print("Element " + element + " was not clickable " + e.getStackTrace());
		}
	}
	...
	public void dragAndDrop(WebElement sourceElement, WebElement destinationElement) throws Throwable {
		try {
			if (sourceElement.isDisplayed() && destinationElement.isDisplayed()) {
				...
				Print("Element was not displayed to drag");
			}
		} catch (StaleElementReferenceException e) {
			Print("Element with " + sourceElement + "or" + destinationElement
					+ "is not attached to the page document " + e.getStackTrace());
		} catch (NoSuchElementException e) {
			Print("Element " + sourceElement + "or" + destinationElement + " was not found in DOM "
					+ e.getStackTrace());
		} catch (Exception e) {
			Print("Error occurred while performing drag and drop operation " + e.getStackTrace());
		}
	}

	// Switching Windows
	public void switchToWindow(WebDriver driver, int Index) throws Throwable{
		try {
			Set<String> Allwindows = driver.getWindowHandles();
			String windowselect = (String) Allwindows.toArray()[Index];
			driver.switchTo().window(windowselect);
		} catch (Exception e) {
			Print("Exception While switching the windows");
		}
	}

	// Switches to the frame using by Frame Id
	public void switchToFrame(int frame) throws Throwable{
		try {
			driver.switchTo().frame(frame);
			Print("Navigated to frame with id " + frame);
		} catch (NoSuchFrameException e) {
			Print("Unable to locate frame with id " + frame + e.getStackTrace());
		} catch (Exception e) {
			Print("Unable to navigate to frame with id " + frame + e.getStackTrace());
		}
	}

	// Switches to the frame using Frame Name
	public void switchToFrame(String frame) throws Throwable{
		try {
			driver.switchTo().frame(frame);
			Print("Navigated to frame with name " + frame);
		} catch (NoSuchFrameException e) {
			Print("Unable to locate frame with id " + frame + e.getStackTrace());
		} catch (Exception e) {
			Print("Unable to navigate to frame with id " + frame + e.getStackTrace());
	...
	public void switchToFrame(WebElement frameElement) throws Throwable{
		try {
			if (isElementPresent(frameElement)) {
				driver.switchTo().frame(frameElement);
				Print("Navigated to frame with element " + frameElement);
			} else {
				Print("Unable to navigate to frame with element " + frameElement);
			}
		} catch (NoSuchFrameException e) {
			Print("Unable to locate frame with element " + frameElement + e.getStackTrace());
		} catch (StaleElementReferenceException e) {
			Print(
					"Element with " + frameElement + "is not attached to the page document" + e.getStackTrace());
		} catch (Exception e) {
			Print("Unable to navigate to frame with element " + frameElement + e.getStackTrace());
		}
	}

	private boolean isElementPresent(WebElement frameElement) throws Throwable{
		// TODO Auto-generated method stub
		return false;
	}

	// Switches to the frame using Frame Inside Frame (Multiple Frame)
	public void switchToFrame(String ParentFrame, String ChildFrame) throws Throwable {
		try {
			driver.switchTo().frame(ParentFrame).switchTo().frame(ChildFrame);
			Print("Navigated to innerframe with id " + ChildFrame + "which is present on frame with id"
					+ ParentFrame);
		} catch (NoSuchFrameException e) {
			System.out
			.println("Unable to locate frame with id " + ParentFrame + " or " + ChildFrame + e.getStackTrace());
		} catch (Exception e) {
			Print("Unable to navigate to innerframe with id " + ChildFrame
	...
	public void switchtoDefaultFrame() throws Throwable{
		try {
			...
			Print("Navigated back to webpage from frame");
		} catch (Exception e) {
			Print("unable to navigate back to main webpage from frame" + e.getStackTrace());
		}
	}
